...
def find_OMCS_match_for_a_coreference_pair(tmp_data, example_id):
        ...
        for NP_P_pair in tmp_pronoun:
            ...
            for edge in NP_P_pair['pronoun_related_edge']:
                ...
                for pair in OMCS_data:
                    if tmp_other_word not in stop_words and verify_match((filter_stop_words(NP, stop_words), [tmp_other_word]), pair[1:]):
                        if 'dobj' in edge[1] or 'nsubj' in edge[1] or 'nmod' in edge[1]:
                            found_match = True
    ...
    return found_match_pair, len(tmp_data), example_id
        ...
        for NP_P_pair in tmp_pronoun:
            ...
            for edge in NP_P_pair['pronoun_related_edge']:
                if edge[0][1] in all_pronouns:
                    ...
                    tmp_other_word = edge[0][1]
                for pair in OMCS_data:
                    if verify_match((NP, tmp_other_word), pair[1:]) and pair[0] in OMCS_edges and edge[1] not in ['case', 'det:qmod']:
