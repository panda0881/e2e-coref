...
class CorefModel(object):
    ...
    def evaluate_pronoun_by_frequency(self, session, evaluation_data, frequent_list):
        data_for_analysis = list()
        ...
        infrequent_all = 0
        infrequent_predict = 0
        infrequent_correct = 0
        ...
        for example_num, (tensorized_example, example) in enumerate(self.eval_data):
            ...
            for i, pronoun_example in enumerate(example['pronoun_info']):
                ...
                if predict_cluster:
                    for NP in predict_cluster:
                        ...
                        #     continue
                        # print(word_index_to_sentence_index[NP[0]])
                        if -1 <= word_index_to_sentence_index[NP[0]] - tmp_pronoun_sentence_index <= 0:

                        # if verify_correct_NP_match(NP, pronoun_example['candidate_NPs'], 'exact'):
                            predict_coreference += 1
                            result_by_pronoun_type[current_pronoun_type]['predict_coreference'] += 1
                            tmp_NP_words = ' '.join(all_sentence[NP[0]:NP[1] + 1])
                            if tmp_NP_words in frequent_list:
                                frequent_predict += 1
                            else:
                                infrequent_predict += 1
                            if verify_correct_NP_match(NP, pronoun_example['correct_NPs'], 'exact'):
                                correct_predict_coreference += 1
                                result_by_pronoun_type[current_pronoun_type]['correct_predict_coreference'] += 1
                                if tmp_NP_words in frequent_list:
                                    ...
                                    infrequent_correct += 1
                for tmp_gold_np in pronoun_example['correct_NPs']:
                    tmp_NP_words = ' '.join(all_sentence[tmp_gold_np[0]:tmp_gold_np[1] + 1])
                    if tmp_NP_words in frequent_list:
                        frequent_all += 1
                    else:
                        infrequent_all += 1
            ...
            if (example_num+1) % 10 == 0:
                ...
                # print('predict_coreference', predict_coreference)
                # print('all_coreference', all_coreference)
                p = infrequent_correct / infrequent_predict
                r = infrequent_correct / infrequent_all
                if p == 0 and r == 0:
                    f1 = 0
                else:
                    f1 = 2 * p * r / (p + r)
                print("Average infrequent F1 (py): {:.2f}%".format(f1 * 100))
                print("Average infrequent precision (py): {:.2f}%".format(p * 100))
                print("Average infrequent recall (py): {:.2f}%".format(r * 100))

                p = frequent_correct / frequent_predict
                r = frequent_correct / frequent_all
                if p == 0 and r == 0:
                    f1 = 0
                else:
                    f1 = 2 * p * r / (p + r)
                print("Average frequent F1 (py): {:.2f}%".format(f1 * 100))
                print("Average frequent precision (py): {:.2f}%".format(p * 100))
                print("Average frequent recall (py): {:.2f}%".format(r * 100))
